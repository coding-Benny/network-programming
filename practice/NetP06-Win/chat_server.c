...
int sharewith[MAXCLIENTS];	// 파일 공유 연결 정보 보관
int k = -1;
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				if (strncmp(buf2, CHAT_CMD_FILE_NAME, strlen(CHAT_CMD_FILE_NAME)) == 0) {
					...
					printf("[파일 정보 전달] from %s to %s filename=%s filesize=%d\n", buf1, userlist[sharewith[i]], filename, filesize);
				...
				if (strncmp(buf2, CHAT_CMD_FILE_NO, strlen(CHAT_CMD_FILE_NO)) == 0) {
					for (j = 0; j < num_chat; j++) {
						if (strncmp(buf3, userlist[j], strlen(userlist[j])) == 0) {
							sprintf(buf, "[%s]님이 파일 수신을 거부했습니다.\n", userlist[i]);
							if (send(client_fds[j], buf, BUF_LEN, 0) < 0) {
								printf("client[%d] send error.", i);
								client_error[i] = 1;
								continue;
							}
							printf("[파일 수신 거부] from %s to %s", buf1, buf3);
							break;
						}
					}
					continue;
				...
				if (strncmp(buf2, "\n", 1) == 0) {
					usersleep[i] = 0;	// enter key치면 깨어난다.
					continue;
				}
				...
				else if (userwith[i] != -1) {	/* 1:1 대화 전송 */
